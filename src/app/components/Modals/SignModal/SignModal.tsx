    ...
    return {
      ...
      getStep1Details: () => ({
        "Finality Provider": formatValue(
          transactionData.finalityProviderPK,
          formatPK,
        ),
        "Staking Amount": formatValue(
          transactionData.stakingAmount,
          formatBtcAmount,
        ),
        "Staking Timelock": formatValue(
          transactionData.stakingTimelock,
          formatBlocks,
        ),
      }),
      ...
      getStep2Details: () => {
        const details: Record<string, string> = {
          "Unbonding Timelock": formatValue(
            transactionData.unbondingTimelock,
            formatBlocks,
          ),
          "Covenant Quorum": formatValue(
            transactionData.covenantQuorum,
            formatNumber,
          ),
        };

        // Add individual covenant public keys
        if (transactionData.covenantNoCoordPks?.length) {
          details["Covenant PKs Count"] = formatValue(
            transactionData.covenantNoCoordPks.length,
            (count) => `${formatNumber(count)} keys`,
          );

          // Add each covenant public key with its index
          transactionData.covenantNoCoordPks.forEach((pk, index) => {
      ...
      getStep3Details: () => ({
        "BABY Address": formatValue(transactionData.bbnAddress),
      }),
