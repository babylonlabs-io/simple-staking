...
export const ERROR_MESSAGES: Record<ErrorCodeType, string> = {
  ...
  [ErrorCodes.CLIENT_GENERIC]: "A client-side error occurred.",
  ...
  [ErrorCodes.SERVER_TOO_MANY_REQUESTS]:
    "Too many requests. Please wait a moment before try again.",
  [ErrorCodes.SERVER_UNPROCESSABLE_ENTITY]:
    "The server could not process your request. Please check your input.",

  // Fallback error message
  [ErrorCodes.UNKNOWN]: "An unknown error occurred. Please try again later.",
};

export function getErrorMessage(code: ErrorCodeType, details?: string): string {
  const baseMessage =
    ERROR_MESSAGES[code] || ERROR_MESSAGES[ErrorCodes.UNKNOWN];
