...
import { nativeSegwitMainnetUTXOs } from "../mock/mainnet/wallet/nativeSegwit/utxos";
import { taprootMainnetUTXOs } from "../mock/mainnet/wallet/taproot/utxos";
import { nativeSegwitSignetUTXOs } from "../mock/signet/wallet/nativeSegwit/utxos";
import { taprootSignetUTXOs } from "../mock/signet/wallet/taproot/utxos";
...
export class BTCWallet extends WalletProvider {
  ...
  constructor(
    ...
    // Adjust the derivation path based on the network and wallet type
    let path = walletType === "taproot" ? "m/86'/0'/0'/0/0" : "m/84'/0'/0'/0/0"; // Taproot or NativeSegwit - Mainnet by default
    if (network === Network.TESTNET || network === Network.SIGNET) {
      path = walletType === "taproot" ? "m/86'/1'/0'/0/0" : "m/84'/1'/0'/0/0"; // Testnet and Signet derivation path
    }
